# ════════════════════════════════════════════════════════════════════════════
# PLAYIFY BOT - GUI Backend (Optimized for Performance)
...
# ─── THIRD-PARTY: UTILITIES & PERFORMANCE ──────────────────────────────────
...
APP_DATA_DIR: str = os.path.join(os.getenv("LOCALAPPDATA") or "", "Playify")
DB_PATH: str = os.path.join(APP_DATA_DIR, "playify_state.db")
CONFIG_PATH: str = os.path.join(APP_DATA_DIR, "playify_config.env")
LOGS_PATH: str = os.path.join(APP_DATA_DIR, "logs")
os.makedirs(APP_DATA_DIR, exist_ok=True)

# ─── CACHE SETTINGS (Performance Optimization) ────────────────────────────
CACHE_MAX_SIZE: int = 75_000  # URL/metadata cache size
CACHE_TTL_METADATA: int = 7_200  # 2 hours
CACHE_TTL_STREAM: int = 300  # 5 minutes

# ─── TIMEOUT SETTINGS (Performance) ────────────────────────────────────────
TIMEOUT_VIEW_PAGINATION: float = 300.0  # 5 minutes for pagination views
TIMEOUT_VIEW_FILTER: float = 180.0  # 3 minutes for filter views
TIMEOUT_VIEW_SEEK: float = 180.0  # 3 minutes for seek views
TIMEOUT_HTTP_REQUEST: float = 10.0  # 10 seconds for HTTP requests
TIMEOUT_VOICE_CONNECTION: int = 300  # 5 minutes before auto-disconnect

# ─── PAGINATION & DISPLAY ─────────────────────────────────────────────────
ITEMS_PER_PAGE: int = 25  # Spotify playlist pagination
SEARCH_LIMIT_YOUTUBE: int = 100  # YouTube search limit
SEARCH_LIMIT_SPOTIFY: int = 50  # Spotify search limit
SEARCH_LIMIT_SOUNDCLOUD: int = 10  # SoundCloud search limit
HTTP_REDIRECT_TIMEOUT: int = 10  # Redirect check timeout

# ─── RESTART & RELIABILITY ────────────────────────────────────────────────
RESTART_THRESHOLD: int = 5  # Max restart attempts
MAX_QUEUE_SIZE: int = 5_000  # Prevent unbounded queue growth

# ─── REGEX PATTERNS ───────────────────────────────────────────────────────
ARTIST_NOISE_PATTERNS: Tuple[str, ...] = (
    r"^\[.*?\]\s*",
    r"\s*\(feat\..*?\)",
    r"\s*\(remix\)",
)
...
# ════════════════════════════════════════════════════════════════════════════
# DIAGNOSTIC FUNCTIONS (NaCl Import Test)
# ════════════════════════════════════════════════════════════════════════════


def run_nacl_diagnostics() -> None:
    """Run diagnostics for PyNaCl cffi compatibility."""
    app_data_dir = os.path.join(os.getenv("LOCALAPPDATA") or "", "Playify")
    ...
    with open(log_file_path, "w", encoding="utf-8") as f:
        ...
        try:
            ...
            f.write(
                "Tentative de création d'un objet SecretBox (test de la librairie C)...\n"
            )
...
def _save_all_states_sync():
    ...
    """
    logger.info(
        "Executing synchronous state save to the database in a background thread..."
    )
    conn = None
    try:
        conn = sqlite3.connect(DB_PATH)
        ...
        for guild_id, is_kawaii in kawaii_mode.items():
            ...
            cursor.execute(
                "INSERT OR REPLACE INTO guild_settings (guild_id, kawaii_mode, controller_channel_id, controller_message_id, is_24_7, autoplay, volume) VALUES (?, ?, ?, ?, ?, ?, ?)",
                settings,
            )
        ...
        for guild_id, channels in allowed_channels_map.items():
            for channel_id in channels:
                cursor.execute(
                    "INSERT OR REPLACE INTO allowlist (guild_id, channel_id) VALUES (?, ?)",
                    (guild_id, channel_id),
                )
        ...
        for guild_id, player in music_players.items():
            ...
            cursor.execute(
                "INSERT OR REPLACE INTO playback_state (guild_id, voice_channel_id, current_song_json, queue_json, history_json, radio_playlist_json, loop_current, playback_timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                state_data,
            )
        ...
        logger.error(
            f"An error occurred during the synchronous database save: {e}",
            exc_info=True,
        )
        if conn:
...
messages = {
    ...
    "lyrics_refine_button": {"normal": "Refine Search", "kawaii": "Try again! (o･ω･)ﾉ"},
    ...
    "karaoke_retry_button": {"normal": "Refine Search", "kawaii": "Try Again! (o･ω･)ﾉ"},
    ...
    "controller_title": {
        "normal": "Music Controller",
        "kawaii": "Music Controller (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧",
    },
    ...
    "controller_lyrics_label": {"normal": "Lyrics", "kawaii": "Lyrics (づ￣ ³￣)づ"},
    "controller_karaoke_label": {"normal": "Karaoke", "kawaii": "Karaoke 🎤(°▽°)"},
...
def ydl_worker(ydl_opts, query, cookies_file=None):
    ...
    It performs the yt-dlp extraction and returns a status dictionary.
    """
    ...
    try:
        ...
        # Convertir l'exception en une chaîne de caractères simple est sûr pour les threads.
        return {"status": "error", "message": str(e)}


async def process_spotify_url(url, guild_id):
    """
    ...
    config_file_path = os.path.join(
        os.getenv("LOCALAPPDATA"), "Playify", "playify_config.env"
    )
...
async def process_deezer_url(url, guild_id):
    try:
...
async def process_apple_music_url(url, guild_id):
...
async def process_tidal_url(url, guild_id):
...
async def process_amazon_music_url(url, guild_id):
...
def _async_worker(func, *args):
    """A top-level helper function that can be serialized to execute an asynchronous function in a process."""
    import asyncio

    return asyncio.run(func(*args))


async def run_in_executor(loop, func, *args):
    """Helper to run synchronous or heavy async functions in the process pool."""
    if asyncio.iscoroutinefunction(func):
        return await loop.run_in_executor(process_pool, _async_worker, func, *args)
    else:
...
def run_bot(status_queue, log_queue, command_queue):
    ...
    # Configure the root logger to use our stream queue
    logging.basicConfig(
        level=logging.INFO,
    ...
    # --- NOUVEAU BLOC DE CHARGEMENT OPUS (PLUS ROBUSTE) ---
    try:
        if not discord.opus.is_loaded():
            # Déterminer le chemin de base (fonctionne pour le script et l'exe compilé)
            if getattr(sys, "frozen", False):
                # Si l'application est "gelée", le chemin de base est le dossier temporel _MEIPASS
                base_path = sys._MEIPASS
            else:
                # Sinon, c'est le dossier où se trouve le script
                base_path = os.path.dirname(os.path.abspath(__file__))

            # Construire le chemin complet vers la DLL
            # Assurez-vous que le nom du fichier est EXACTEMENT celui que vous avez.
            # J'ai vu "libopus-0.dll" dans votre titre, donc j'utilise ça. Adaptez si besoin.
            opus_lib_name = "libopus-0.x64.dll"  # ou 'libopus-0.dll'
            opus_path = os.path.join(base_path, opus_lib_name)

            print(f"Attempting to load Opus from: {opus_path}")

            if os.path.exists(opus_path):
                discord.opus.load_opus(opus_path)
                print(f"Successfully loaded {opus_lib_name}")
        else:
            print(f"!!! CRITICAL: Opus library not found at path: {opus_path}")
            raise discord.opus.OpusError("DLL file not found at the specified path.")
    ...
    # --- NEW: Initialize the ProcessPoolExecutor here ---
    # This is critical for compatibility with multiprocessing.
    global process_pool
    try:
    ...
    if not os.path.exists(config_file_path):
        log_queue.put(f"ERROR: Configuration file does not exist at {config_file_path}")
        status_queue.put("ERROR: CONFIG FILE MISSING")
        return

    # 3. Charger les variables d'environnement depuis le chemin absolu
    load_dotenv(dotenv_path=config_file_path)
        ...
        while not bot.is_closed():  # As long as the bot is running
            try:
                ...
                if command in ["RESTART", "QUIT"]:
                    logger.info(
                        f"Command received: {command}. Shutting down gracefully."
                    )
    ...
    async def save_all_states():
        ...
        """
        logger.info(
            "Scheduling the blocking database save to run in the thread executor..."
        )
        try:
            loop = asyncio.get_running_loop()
            ...
            logger.critical(
                f"Failed to schedule or execute the state save task: {e}", exc_info=True
            )
    ...
    async def load_states_on_startup():
        ...
        # Load settings
        cursor.execute(
            "SELECT guild_id, kawaii_mode, is_24_7, autoplay, volume FROM guild_settings"
        )
            ...
            try:
                ...
                if row["voice_channel_id"] and player.current_info:
                    ...
                    if channel and isinstance(channel, discord.VoiceChannel):
                        ...
                        player.text_channel = bot.get_channel(
                            controller_channels.get(
                                guild_id,
                                (
                                    channel.last_message.channel.id
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if play_command:
                ...
                await interaction.response.send_message(
                    "Error: Could not find the play command.", ephemeral=True
                )
    ...
    class MusicControllerView(View):
        ...
        def update_buttons(self):
            ...
            for child in self.children:
                ...
                if custom_id == "controller_pause":
                    child.label = (
                        get_label("controller_resume_label")
                        if is_paused
                        else get_label("controller_pause_label")
                    )
            ...
            pause_button = discord.utils.get(
                self.children, custom_id="controller_pause"
            )
            if pause_button:
                pause_button.style = (
        ...
        async def previous_button(
            ...
            logger.warning(
                "=" * 20
                + f" [DEBUG-PREVIOUS] INITIATED in Guild {guild_id} "
                + "=" * 20
            )
            history_before = [
            ...
            async with music_player.queue_lock:
                if len(music_player.history) < 2:
                    ...
                    return await interaction.response.send_message(
                        "No previous song in history.", ephemeral=True, silent=True
                    )
                ...
                popped_previous_title = get_track_display_info(
                ...
                if (
                    ...
                    logger.error(
                        f"[DEBUG-PREVIOUS] POTENTIAL BUG: Queue size mismatch!"
                    )
        ...
        @discord.ui.button(
            style=ButtonStyle.secondary, custom_id="controller_pause", row=0
        )
        ...
        @discord.ui.button(
            style=ButtonStyle.primary, custom_id="controller_skip", row=0
        )
        ...
        @discord.ui.button(
            style=ButtonStyle.secondary, custom_id="controller_loop", row=1
        )
        ...
        async def lyrics_button(self, interaction: discord.Interaction, button: Button):
            ...
            if lyrics_command:
                ...
                await interaction.response.send_message(
                    "Lyrics command not found.", ephemeral=True, silent=True
                )

        @discord.ui.button(
            style=ButtonStyle.secondary, custom_id="controller_karaoke", row=2
        )
        async def karaoke_button(
            self, interaction: discord.Interaction, button: Button
        ):
            ...
            if karaoke_command:
                ...
                await interaction.response.send_message(
                    "Karaoke command not found.", ephemeral=True, silent=True
                )

        @discord.ui.button(
            style=ButtonStyle.primary, custom_id="controller_queue", row=2
        )
        ...
        async def jump_to_song_button(
            ...
            if jumpto_command:
                ...
                await interaction.response.send_message(
                    "Jump To command not found.", ephemeral=True, silent=True
                )
    ...
    async def create_status_embed(guild_id: int) -> Embed:
        ...
        if _24_7_active.get(guild_id, False):
            ...
            status_lines.append(
                get_messages("queue_status_24_7", guild_id).format(mode=mode_24_7)
            )
    ...
    async def create_controller_embed(bot, guild_id):
        ...
        if not is_playing:
            ...
            embed.set_image(
                url="https://image2url.com/images/1764519755428-cad6bced-fa34-4c61-9d01-4fb741893f6e.png"
            )
        ...
        is_24_7_normal = (
            _24_7_active.get(guild_id, False) and not music_player.autoplay_enabled
        )

        queue_snapshot = []
        if is_24_7_normal and music_player.radio_playlist:
            current_url = (
                music_player.current_info.get("url")
                if music_player.current_info
                else None
            )
            try:
                current_index = [
                    t.get("url") for t in music_player.radio_playlist
                ].index(current_url)
                queue_snapshot = (
        ...
        tracks_to_hydrate = [
            ...
            if isinstance(t, dict) and not t.get("hydrated", False)
        ]
        if tracks_to_hydrate:
            tasks = [fetch_meta(track["url"], None) for track in tracks_to_hydrate]
            hydrated_results = await asyncio.gather(*tasks)
        ...
        if tracks_to_display:
            ...
            next_title = display_info.get("title")
            # Truncate next song title
            if len(next_title) > 60:
                next_title = next_title[:57] + "..."
            next_duration, next_url = format_duration(
                display_info.get("duration")
            ), display_info.get("webpage_url")

            if display_info.get("source_type") == "lazy":
                next_song_text = f"`{next_title}`"
        ...
        # Safely build the queue description field to avoid exceeding character limit
        description_lines = []
        current_length = 0
        MAX_DESC_LENGTH = 900  # Safe limit below Discord's 1024 cap

        if len(tracks_to_display) > 1:
            for i, item in enumerate(tracks_to_display[1:], start=2):
                display_info = get_track_display_info(item)
                item_title = display_info.get("title")
                if len(item_title) > 40:
                    item_title = item_title[:37] + "..."
                item_duration = format_duration(display_info.get("duration"))

                line = ""
                if display_info.get("source_type") == "file":
                    line = f"`{i}.` 💿 `{item_title}` - `{item_duration}`"
                elif display_info.get("source_type") == "lazy":
                    line = f"`{i}.` {item_title}"
                else:
                    line = f"`{i}.` {item_title} - `{item_duration}`"

                if current_length + len(line) > MAX_DESC_LENGTH:
                    description_lines.append(
                        f"\n... and {len(queue_snapshot) - i + 1} more songs."
                    )
                    break
            ...
            description_lines.append(
                get_messages("controller_no_other_songs", guild_id)
            )
        else:
            description_lines.append(
                get_messages("controller_queue_is_empty", guild_id)
            )
        ...
        embed.add_field(
            name=get_messages("controller_next_up_field", guild_id),
        ...
        embed.add_field(
            name=get_messages("controller_now_playing_field", guild_id),
        ...
        KAOMOJI_PLATFORM_DISPLAY = {
            ...
            "Apple Music": "Apple Music (≧◡≦)",
            "Tidal": "Tidal (〜￣▽￣)〜",
            "Amazon Music": "Amazon Music (b ᵔ▽ᵔ)b",
        ...
        if active_filters:
            ...
            display_name = FILTER_DISPLAY_NAMES.get(
                filter_name, filter_name.capitalize()
            )
            dynamic_footer_info = f"Filter: {display_name}" + (
                " ✨" if is_kawaii else ""
            )
        elif music_player.current_info:
            source_type = music_player.current_info.get("source_type")
            current_display_map = (
                KAOMOJI_PLATFORM_DISPLAY if is_kawaii else PLATFORM_DISPLAY
            )

            if source_type == "file":
                dynamic_footer_info = "Source: Local File" + (
                    " (`•ω•´)" if is_kawaii else " 💿"
                )
            else:
                ...
                if original_platform and original_platform in current_display_map:
                    dynamic_footer_info = (
                        f"Source: {current_display_map[original_platform]}"
                    )
        ...
        if count_for_display == 0 and info:
            ...
            if is_kawaii:
                last_song_format = (
                    "Last song!~ | {dynamic_info} | Vol: {volume}% (´• ω •`)"
                )
            footer_text = last_song_format.format(
                dynamic_info=dynamic_footer_info, volume=int(music_player.volume * 100)
            )
    ...
    class SeekModal(discord.ui.Modal):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if target_seconds is None:
                await interaction.response.send_message(
                    get_messages("seek_fail_invalid_time", self.view.guild_id),
                    ephemeral=True,
    ...
    class SeekView(View):
        ...
        def __init__(self, interaction: discord.Interaction):
            ...
            # Apply button labels
            self.rewind_button.label = get_messages(
                "rewind_button_label", self.guild_id
            )
            self.jump_button.label = get_messages("jump_to_button_label", self.guild_id)
            self.forward_button.label = get_messages(
                "fastforward_button_label", self.guild_id
            )
            ...
            for i, video in enumerate(search_results):
                options.append(
                    discord.SelectOption(
                        ...
                        description=f"by {video.get('uploader', 'Unknown Artist')}"[
                            :100
                        ],
            ...
            try:
                ...
                if guild_id not in controller_channels:
                    ...
                    await interaction.followup.send(
                        f"✅ Added to queue: {video_info.get('title', 'Unknown Title')}",
                        ephemeral=True,
                        silent=SILENT_MESSAGES,
                    )
    ...
    class LyricsView(View):
        ...
        async def previous_button(
            ...
            await interaction.response.edit_message(
                embed=self.update_embed(), view=self
            )
        ...
        @discord.ui.button(
            label="Refine", emoji="✏️", style=discord.ButtonStyle.secondary, row=0
        )
    ...
    class LyricsRetryModal(discord.ui.Modal, title="Refine Lyrics Search"):
        def __init__(
            self, original_interaction: discord.Interaction, suggested_query: str
        ):
            super().__init__()
            ...
            try:
                ...
                if not song:
                    fail_message = get_messages(
                        "lyrics_not_found_description", self.guild_id
                ...
                await interaction.followup.send(
                    "Lyrics found!", silent=SILENT_MESSAGES, ephemeral=True
                )

            except Exception as e:
                logger.error(f"Error during lyrics retry: {e}")
                await interaction.followup.send(
                    "An error occurred during the new search.",
                    silent=SILENT_MESSAGES,
                    ephemeral=True,
                )
    ...
    class KaraokeRetryModal(discord.ui.Modal, title="Refine Karaoke Search"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if not lrc:
                fail_message = get_messages("karaoke_retry_fail", self.guild_id).format(
                    query=new_query
                )
                await interaction.followup.send(
    ...
    class RefineLyricsModal(discord.ui.Modal, title="Refine Lyrics Search"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            try:
                ...
                if not song:
                    await interaction.followup.send(
                        f"Sorry, I still couldn't find lyrics for **{new_query}**.",
                        silent=SILENT_MESSAGES,
                        ephemeral=True,
                    )
                    return
    ...
    class KaraokeRetryView(discord.ui.View):
        def __init__(
            ...
            self.retry_button.label = get_messages(
                "karaoke_retry_button", self.guild_id
            )
            self.genius_fallback_button.label = get_messages(
                "karaoke_genius_fallback_button", self.guild_id
            )
    ...
    class QueueView(View):
        ...
        def __init__(
            ...
            self.previous_button = Button(
                label=get_messages("previous_button", self.guild_id),
                style=ButtonStyle.secondary,
            )
            self.next_button = Button(
                label=get_messages("next_button", self.guild_id),
                style=ButtonStyle.secondary,
            )
        ...
        async def create_queue_embed(self) -> Embed:
            ...
            if _24_7_active.get(self.guild_id, False):
                ...
                status_lines.append(
                    get_messages("queue_status_24_7", self.guild_id).format(
                        mode=mode_24_7
                    )
                )
            elif self.music_player.autoplay_enabled:
                status_lines.append(
                    get_messages("queue_status_autoplay", self.guild_id)
                )
            ...
            if len(self.tracks) == 0 and self.music_player.current_info:
                ...
                description_text = get_messages(
                    "queue_description", self.guild_id
                ).format(count=len(self.tracks))
            ...
            if self.tracks:
                ...
                if tracks_to_hydrate:
                    tasks = [
                        fetch_meta(track["url"], None) for track in tracks_to_hydrate
                    ]
                    hydrated_results = await asyncio.gather(*tasks)
                    hydrated_map = {res["url"]: res for res in hydrated_results if res}
                    for track in tracks_on_page:
                        if isinstance(track, dict) and track.get("url") in hydrated_map:
                            ...
                            track["webpage_url"] = new_data.get(
                ...
                if next_songs_list:
                    embed.add_field(
                        ...
                        value="".join(next_songs_list),
                        inline=False,
                    )

            embed.set_footer(
                text=get_messages("queue_page_footer", self.guild_id).format(
                    current_page=self.current_page + 1, total_pages=self.total_pages
                )
            )
            return embed
    ...
    class RemoveSelect(discord.ui.Select):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            embed = Embed(
                title=get_messages("remove_success_title", guild_id).format(
    ...
    async def play_silence_loop(guild_id: int):
        ...
        try:
            ...
            logger.info(
                f"[{guild_id}] Silence loop task cancelled, proceeding to cleanup."
            )
            pass
        except Exception as e:
            ...
            if vc and vc.is_connected() and music_player.is_playing_silence:
                logger.info(
                    f"[{guild_id}] Scheduling final cleanup for silence source."
                )
    ...
    async def ensure_voice_connection(
        ...
        # --- STANDARD OPERATIONS ON A HEALTHY CLIENT ---
            ...
            logger.info(
                f"[{guild_id}] Moving to a new voice channel: {voice_channel.name}"
            )
        ...
        if vc and (vc.is_playing() or vc.is_paused()):
            ...
            if isinstance(vc.source, discord.PCMAudio) and hasattr(
                ...
                try:
                    ...
                    logger.error(
                        f"[{vc.guild.id}] Error killing FFMPEG in safe_stop: {e}"
                    )
    ...
    def create_queue_item_from_info(info: dict) -> dict:
        ...
        return {
            ...
            "source_type": info.get(
                "source_type"
            ),  # Preserve for other potential types
            "requester": info.get("requester"),
        }
    ...
    async def fetch_and_display_genius_lyrics(
        ...
        if not genius:
            return await interaction.followup.send(
                "Genius API is not configured.", silent=SILENT_MESSAGES, ephemeral=True
            )
        ...
        try:
            ...
            if not song:
                logger.info(
                    f"Precise Genius search failed, trying broad search: '{clean_title}'"
                )
                song = await asyncio.wait_for(
            ...
            if not song:
                ...
                error_desc = get_messages(
                    "lyrics_not_found_description", guild_id
                ).format(query=precise_query)
            ...
            for line in lines:
                if (
                    "contributor" in line.lower()
                    or "lyrics" in line.lower()
                    or "embed" in line.lower()
                ):
                    continue
            ...
            await interaction.followup.send(
                "Sorry, the lyrics search took too long to respond. Please try again later.",
                silent=SILENT_MESSAGES,
                ephemeral=True,
            )
        except Exception as e:
            logger.error(
                f"Error fetching/displaying Genius lyrics for '{clean_title}': {e}"
            )
            await interaction.followup.send(
                "An error occurred while displaying the lyrics.",
                silent=SILENT_MESSAGES,
                ephemeral=True,
            )
    ...
    def create_loading_bar(progress, width=10):
        ...
        return (
            "```[" + "█" * filled + "░" * unfilled + "] " + f"{int(progress * 100)}%```"
        )
    ...
    def parse_yt_dlp_error(error_string: str) -> tuple[str, str, str]:
        ...
        if (
            "sign in to confirm your age" in error_lower
    ...
    async def play_audio(
        guild_id, seek_time=0, is_a_loop=False, song_that_just_ended=None
    ):
        music_player = get_player(guild_id)
        is_kawaii = get_mode(guild_id)
            ...
            if music_player.manual_stop:
                ...
                bot.loop.create_task(
                    play_audio(
                        guild_id,
                        is_a_loop=False,
                        song_that_just_ended=song_that_finished,
                    )
                )
                return
        ...
        try:
            if not (is_a_loop or seek_time > 0):
                    ...
                    if (
                        ...
                        if seed_source_info:
                            ...
                            if IS_PUBLIC_VERSION and (
                                "youtube.com" in url_to_test
                                or "youtu.be" in url_to_test
                            ):
                                url_to_test = ""
                            ...
                            if any(
                                ...
                                if music_player.text_channel:
                                    try:
                                        notice_key = (
                                            "autoplay_file_notice"
                                            if seed_source_info.get("source_type")
                                            == "file"
                                            else "autoplay_direct_link_notice"
                                        )
                                        notice_embed = Embed(
                        ...
                        if seed_url:
                            ...
                            try:
                                if not progress_message and music_player.text_channel:
                                    initial_embed = Embed(
                                        title=get_messages(
                                            "autoplay_loading_title", guild_id
                                        ),
                                        description=get_messages(
                ...
                if isinstance(next_item, LazySearchItem):
                    logger.info(
                        f"[{guild_id}] Lazy track detected, initiating resolution."
                    )
                        ...
                        if music_player.text_channel:
                            try:
                                error_embed = Embed(
                                    ...
                                    color=(
                                        0xFF9AA2 if is_kawaii else discord.Color.red()
                                    ),
                                )
                                await music_player.text_channel.send(
                ...
                try:
                    ...
                    if music_player.text_channel:
                        try:
                            ...
                            embed.add_field(
                                name="Affected URL", value=f"`{url_for_fetching}`"
                            )
                            await music_player.text_channel.send(
            ...
            filter_chain = (
                ",".join(
                    [AUDIO_FILTERS[f] for f in active_filters if f in active_filters]
                )
                if active_filters
                else ""
            )
            ...
            try:
                ...
                if current_line_index != last_line_index:
                    ...
                    new_description = format_lyrics_display(
                        music_player.synced_lyrics, current_line_index
                    )

                    if (
                        music_player.lyrics_message
                        and music_player.lyrics_message.embeds
                    ):
        ...
        if music_player.lyrics_message:
            try:
                await music_player.lyrics_message.edit(
                    content="*Karaoke session finished!*", embed=None, view=None
                )
            except discord.NotFound:
                pass
    ...
    async def lyrics(interaction: discord.Interaction):
        ...
        if (
            ...
            return await interaction.response.send_message(
                "No music is currently playing.", silent=SILENT_MESSAGES, ephemeral=True
            )
    ...
    async def karaoke(interaction: discord.Interaction):
        ...
        if music_player.lyrics_task and not music_player.lyrics_task.done():
            return await interaction.response.send_message(
                "Lyrics are already being displayed!",
                silent=SILENT_MESSAGES,
                ephemeral=True,
            )
        ...
        async def proceed_with_karaoke():
            ...
            try:
                precise_query = f"{clean_title} {artist_name}"
                logger.info(
                    f"Attempting precise synced lyrics search: '{precise_query}'"
                )
                lrc = await asyncio.wait_for(
            ...
            if not lyrics_lines:
                ...
                error_desc = get_messages(
                    "karaoke_not_found_description", guild_id
                ).format(query=f"{clean_title} {artist_name}")
            ...
            music_player.lyrics_task = asyncio.create_task(
                update_karaoke_task(guild_id)
            )
    ...
    async def play(interaction: discord.Interaction, query: str):
        ...
        async def add_and_update_controller(info: dict):
            """Helper to add a single, fully-resolved track to the queue."""
            queue_item = {
                ...
                "is_single": True,
                "requester": interaction.user,
                "hydrated": True,  # Mark as fully loaded
            }
            await music_player.queue.put(queue_item)
        ...
        async def handle_platform_playlist(platform_tracks, platform_name):
            """Helper for Spotify, Deezer, etc. playlists, which are pre-processed."""
            ...
            embed = Embed(
                ...
                description=get_messages(desc_key, guild_id).format(
                    count=total_tracks, failed=0, failed_tracks=""
                ),
                color=0xB5EAD7 if is_kawaii else discord.Color.green(),
            )
            await interaction.followup.send(silent=SILENT_MESSAGES, embed=embed)
        ...
        try:
            ...
            if spotify_regex.match(query):
                ...
                platform_processor, platform_name = (
                    process_apple_music_url,
                    "Apple Music",
                )
                ...
                platform_processor, platform_name = (
                    process_amazon_music_url,
                    "Amazon Music",
                )

            if platform_processor:
                loop = asyncio.get_running_loop()
                platform_tracks = await run_in_executor(
                    loop, platform_processor, query, guild_id
                )
                ...
                if platform_tracks is None:
                    error_key_map = {
                        ...
                        "Deezer": "deezer_error",
                        "Apple Music": "apple_music_error",
                        "Tidal": "tidal_error",
                        "Amazon Music": "amazon_music_error",
                    }
                    error_message_key = error_key_map.get(platform_name, "video_error")
                    embed = Embed(
                        description=get_messages(error_message_key, guild_id),
                        color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                    )
                    await interaction.followup.send(
            ...
            if direct_platform_regex.match(query) or direct_link_regex.match(query):
                ...
                if "youtube.com" in query or "youtu.be" in query:
                    ...
                    if "v" in query_params and "list" in query_params:
                        ...
                        if playlist_id.startswith("RD"):
                            logger.info(
                                f"[{guild_id}] YouTube Mix detected. Using original URL to extract all tracks: '{url_to_fetch}'"
                            )
                            ...
                            logger.info(
                                f"[{guild_id}] Standard YouTube Playlist detected. Using clean playlist URL: '{clean_playlist_url}'"
                            )
                ...
                logger.info(
                    f"[{guild_id}] Using fast, flat extraction for direct link/playlist: {url_to_fetch}"
                )
                ydl_opts_override = {"extract_flat": True, "noplaylist": False}
                info = await fetch_video_info_with_retry(
                    url_to_fetch, ydl_opts_override=ydl_opts_override
                )
                ...
                if "entries" in info and info["entries"]:
                    ...
                    logger.info(
                        f"[{guild_id}] Adding {len(tracks_to_add)} unhydrated tracks from a direct playlist."
                    )
                    ...
                    embed = Embed(
                        ...
                        description=get_messages(
                            "playlist_description", guild_id
                        ).format(count=len(tracks_to_add)),
                        color=0xB5EAD7 if is_kawaii else discord.Color.green(),
                    )
                    await interaction.followup.send(embed=embed, silent=SILENT_MESSAGES)
                else:
            ...
            if interaction.response.is_done():
                try:
                    await interaction.edit_original_response(
                        content=f"An error occurred: {str(e)[:1800]}",
                        embed=None,
                        view=None,
                    )
    ...
    @bot.tree.command(
        name="play-files",
        description="Plays one or more uploaded audio or video files.",
    )
    ...
    async def play_files(
        ...
        if not added_files:
            await interaction.followup.send(
                embed=Embed(
                    description="No valid audio/video files were added.",
                    color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                ),
                ephemeral=True,
                silent=SILENT_MESSAGES,
        ...
        description = (
            f"**{len(added_files)} file(s) added to the queue:**\n"
    ...
    async def play_next(
        ...
        if query:
            try:
                ...
                if is_platform_link:
                    tracks = None
                    if spotify_regex.match(query):
                        tracks = await process_spotify_url(query, guild_id)
                    elif deezer_regex.match(query):
                        tracks = await process_deezer_url(query, guild_id)
                    elif apple_music_regex.match(query):
                        tracks = await process_apple_music_url(query, guild_id)
                    elif tidal_regex.match(query):
                        tracks = await process_tidal_url(query, guild_id)
                    elif amazon_music_regex.match(query):
                        tracks = await process_amazon_music_url(query, guild_id)
                    ...
                    if len(tracks) > 1:
                        ...
                        await interaction.followup.send(
                            embed=Embed(
                                description="Playlists and albums are not supported for `/playnext`. Please add them with `/play`.",
                                color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                            ),
                            ephemeral=True,
                            silent=SILENT_MESSAGES,
                ...
                if not (
                    ...
                    logger.info(
                        f"[/playnext] Processing as keyword search: {search_term}"
                    )
                    search_prefix = "scsearch:" if IS_PUBLIC_VERSION else "ytsearch:"
                    search_query = f"{search_prefix}{sanitize_query(search_term)}"
                ...
                logger.error(
                    f"Error processing /playnext for query '{query}': {e}",
                    exc_info=True,
                )
                return
        ...
        # This part for handling local files remains the same
            ...
            if not file.content_type or not (
                ...
                embed = Embed(
                    description="The uploaded file is not a valid audio or video type.",
                    color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                )
                await interaction.followup.send(
                    embed=embed, ephemeral=True, silent=SILENT_MESSAGES
            ...
            try:
                ...
                embed = Embed(
                    description="An error occurred while saving the uploaded file.",
                    color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                )
                await interaction.followup.send(
                    embed=embed, ephemeral=True, silent=SILENT_MESSAGES
        ...
        if queue_item:
            ...
            if queue_item.get("source_type") == "file":
                ...
                description_text = (
                    f"[{queue_item['title']}]({queue_item['webpage_url']})"
                )
    ...
    async def now_playing(interaction: discord.Interaction):
        ...
        if music_player.current_info:
            ...
            if music_player.current_info.get("source_type") == "file":
                ...
                url = music_player.current_info.get(
                    "webpage_url", music_player.current_url
                )
                description_text = get_messages(
                    "now_playing_description", guild_id
    ...
    async def pause(interaction: discord.Interaction):
        ...
        if voice_client and voice_client.is_playing():
            if music_player.playback_started_at:
                ...
                music_player.start_time += (
                    elapsed_since_play * music_player.playback_speed
                )
                music_player.playback_started_at = None
    ...
    @app_commands.describe(
        number="[Optional] The track number in the queue to jump to."
    )
    ...
    async def skip(
        ...
        if number is not None:
            async with music_player.queue_lock:
                ...
                if not (1 <= number <= queue_size):
                    await interaction.followup.send(
                        f"Invalid number. Please provide a track number between 1 and {queue_size}.",
                        ephemeral=True,
                        silent=SILENT_MESSAGES,
                    )
                    return
        ...
        if music_player.loop_current:
            ...
            description_text = get_messages("replay_success_desc", guild_id).format(
                title=title, url=url
            )
            embed = Embed(
        ...
        if next_song_info:
            ...
            if hydrated_next_info.get("source_type") == "file":
                ...
                description_text = get_messages(
                    "now_playing_description", guild_id
                ).format(title=next_title, url=next_url)
        ...
        music_player.manual_stop = (
            True  # Ensure loop/247 logic is bypassed for this skip
        )
        await safe_stop(voice_client)
        ...
        if music_player.voice_client and music_player.voice_client.is_connected():
            ...
            if (
                ...
                try:
                    ...
                    logger.error(
                        f"[{guild_id}] Error killing FFMPEG process on /stop: {e}"
                    )
    ...
    async def status(interaction: discord.Interaction):
        ...
        await interaction.response.defer(
            ephemeral=True
        )  # Defer for a potentially long operation
        ...
        embed = discord.Embed(
            ...
            color=(
                0x2ECC71 if latency < 200 else (0xE67E22 if latency < 500 else 0xE74C3C)
            ),  # Color changes with latency
        )
    ...
    @bot.tree.command(
        name="discord",
        description="Get an invite to the official community and support server.",
    )
    ...
    async def support(interaction: discord.Interaction):
        if not interaction.guild:
            await interaction.response.send_message(
                "This command can be used inside any server.",
                ephemeral=True,
                silent=SILENT_MESSAGES,
            )
            return
    ...
    @app_commands.choices(
        mode=[
            ...
            Choice(
                name="Auto (Adds similar songs when the queue is empty)", value="auto"
            ),
    ...
    async def radio_24_7(interaction: discord.Interaction, mode: str):
        ...
        if mode == "off":
            if not _24_7_active.get(guild_id, False):
                await interaction.followup.send(
                    "24/7 mode was not active.", silent=SILENT_MESSAGES, ephemeral=True
                )
                return
        ...
        if not voice_client:
            if music_player.text_channel:
                await music_player.text_channel.send(
                    "Unable to connect to the voice chat.", silent=SILENT_MESSAGES
                )
            return
                ...
                music_player.radio_playlist.append(
                    {
                        ...
                        "title": music_player.current_info.get(
                            "title", "Unknown Title"
                        ),
        ...
        if not music_player.radio_playlist and mode == "normal":
            await interaction.followup.send(
                "The queue is empty. Add songs before enabling 24/7 normal mode.",
                silent=SILENT_MESSAGES,
                ephemeral=True,
            )
            return
    ...
    async def reconnect(interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                "An error occurred during the reconnect process.",
                silent=SILENT_MESSAGES,
                ephemeral=True,
            )
        finally:
    ...
    async def search(interaction: discord.Interaction, query: str):
        ...
        try:
            ...
            if not search_results:
                embed = Embed(
                    description=get_messages("search_no_results", guild_id).format(
                        query=query
                    ),
                    color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                )
    ...
    async def seek_interactive(interaction: discord.Interaction):
        ...
        if not music_player.voice_client or not (
            ...
            await interaction.response.send_message(
                get_messages("no_playback", guild_id),
                ephemeral=True,
                silent=SILENT_MESSAGES,
            )
            return
        ...
        if music_player.is_current_live:
            await interaction.response.send_message(
                get_messages("seek_fail_live", guild_id),
                ephemeral=True,
                silent=SILENT_MESSAGES,
            )
            return
    ...
    class SetupCommands(app_commands.Group):
        ...
        async def controller(
            ...
            controller_messages[guild_id] = (
                None  # Reset message ID to force creation of a new one
            )

            await interaction.response.send_message(
                f"Music controller channel has been set to {target_channel.mention}.",
                ephemeral=True,
                silent=SILENT_MESSAGES,
            )
    ...
    async def allowlist(
        ...
        if channels:
            ...
            embed = discord.Embed(
                description=get_messages("allowlist_set_success", guild_id).format(
    ...
    async def previous(interaction: discord.Interaction):
        ...
        if not vc or not (vc.is_playing() or vc.is_paused()):
            await interaction.response.send_message(
                "Nothing is playing.", ephemeral=True, silent=SILENT_MESSAGES
            )
            return
        ...
        if len(music_player.history) < 2:
            await interaction.response.send_message(
                "No previous song in history.", ephemeral=True, silent=SILENT_MESSAGES
            )
            return
        ...
        await interaction.followup.send(
            "Skipping back to the previous song.", silent=SILENT_MESSAGES
        )

    @bot.tree.command(
        name="jumpto",
        description="Opens a menu to jump to a specific song in the queue.",
    )
    ...
    async def on_voice_state_update(member, before, after):
        ...
        if is_leaving_event:
            ...
            if not [m for m in bot_channel.members if not m.bot]:
                ...
                if _24_7_active.get(guild_id, False):
                    if (
                        not music_player.silence_task
                        or music_player.silence_task.done()
                    ):
    ...
    async def global_interaction_check(interaction: discord.Interaction) -> bool:
        ...
        description_text = get_messages(
            "command_restricted_description", guild_id
        ).format(bot_name=interaction.client.user.name)
    ...
    async def on_ready():
        ...
        try:
            ...
            # --- DYNAMIC PRESENCE FUNCTION ---
            async def rotate_presence():
                await bot.wait_until_ready()

                # Map string names to discord.ActivityType objects
                activity_type_map = {
                ...
                while not bot.is_closed():
                    try:
                        # Load all presence settings from environment variables (set by the app)
                        presence_type_str = os.getenv("PRESENCE_TYPE", "Playing")
                        rotation_count = int(os.getenv("PRESENCE_ROTATION_COUNT", "3"))
                        delay = int(os.getenv("PRESENCE_ROTATION_DELAY", "15"))

                        custom_statuses = []
                        for i in range(1, rotation_count + 1):
                            status_text = os.getenv(f"PRESENCE_{i}")
                            if status_text:  # Ensure the status is not empty
                                custom_statuses.append(status_text)
                        ...
                        if not custom_statuses:
                            custom_statuses = [
                                ...
                                "/play",
                                "alan7383.github.io/playify",
                            ]

                        # Get the correct activity type object, defaulting to 'playing'
                        activity_type = activity_type_map.get(
                            presence_type_str, discord.ActivityType.playing
                        )

                        for status in custom_statuses:
                            # Check if the bot is still running before changing presence
                            if bot.is_closed():
                                return

                            await bot.change_presence(
                                activity=discord.Activity(
                                    name=status, type=activity_type
                                )
                            )
                            # Use the custom delay from settings

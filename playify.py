...
def clear_all_saved_states():
    """
    Clears all saved states from the database.
    WARNING: This will permanently delete all saved data including:
    - Guild settings (kawaii mode, controller settings, 24/7 mode, etc.)
    - Channel allowlists
    - Playback states (current song, queue, history)
    """
    try:
        conn = sqlite3.connect("playify_state.db")
        cursor = conn.cursor()
        ...
        logger.info("All saved states have been cleared from the database.")
        return True
    except Exception as e:
        logger.error(f"Failed to clear saved states: {e}")
        return False


@bot.tree.command(
    name="clear-database",
    description="[ADMIN] Clears all saved bot data from the database.",
)
@app_commands.default_permissions(administrator=True)
async def clear_database(interaction: discord.Interaction):
    """Admin command to clear all saved states."""
    ...
    if view.value:
        success = clear_all_saved_states()

        if success:
            result_embed = discord.Embed(
                title="✅ Database Cleared",
                description="All saved states have been permanently deleted.",
                color=discord.Color.green(),
            )
        else:
            result_embed = discord.Embed(
                title="❌ Error",
                description="Failed to clear database. Check logs for details.",
                color=discord.Color.red(),
            )
        ...
        cancel_embed = discord.Embed(
            title="Cancelled",
            description="Database clear operation cancelled.",
...
def clean_display_title(title: str) -> str:
    """Cleans song title for UI display."""
    if not title:
    ...
    remove_patterns = [
        r"\[.*?\]",
        r"\(.*?\)",
        r"(?i)official\s*(video|audio|lyric|music)?",
        r"(?i)lyric\s*video",
        r"(?i)music\s*video",
        r"(?i)(hd|4k|uhd|1080p|720p)",
        r"(?i)remastered",
        r"(?i)audio\s*only",
        r"(?i)full\s*song",
        r"(?i)radio\s*edit",
...
async def create_controller_embed(bot, guild_id):
    ...
    if not is_playing:
        ...
        embed.set_image(url="https://i.ibb.co.com/pvzx389L/download.jpg")
    ...
    # TERAPKAN PEMBERSIHAN JUDUL UNTUK TAMPILAN CONTROLLER
    ...
    # TERAPKAN PEMBERSIHAN ARTIST UNTUK TAMPILAN CONTROLLER
    display_artist = clean_display_title(artist)
    ...
    if tracks_to_display:
        ...
        # TERAPKAN PEMBERSIHAN JUDUL UNTUK NEXT UP
        clean_next_title = clean_display_title(next_title)

        next_duration = format_duration(display_info.get("duration"))
        next_url = display_info.get("webpage_url")
        ...
        next_song_text = get_messages(
            format_key,
            guild_id,
            title=clean_next_title,  # Gunakan judul yang sudah dibersihkan
            url=next_url,
    ...
    if len(tracks_to_display) > 1:
        for i, item in enumerate(tracks_to_display[1:5], start=2):
            ...
            # TERAPKAN PEMBERSIHAN JUDUL UNTUK QUEUE ITEMS
            clean_item_title = clean_display_title(item_title)

            item_duration = format_duration(display_info.get("duration"))

            display_title_shortened = (
                (clean_item_title[:38] + "..")
                if len(clean_item_title) > 40
                else clean_item_title
            )
            ...
            display_line = get_messages(
                line_key,
                guild_id,
                title=display_title_shortened,  # Gunakan judul yang sudah dibersihkan dan dipendekkan
    ...
    # GUNAKAN DISPLAY TITLE YANG SUDAH DIBERSIHKAN UNTUK NOW PLAYING
    now_playing_title_display = (
        f"**[{display_title}]({info.get('webpage_url', info.get('url', '#'))})**"
        if info.get("source_type") != "file"
        else get_messages(
            "queue.now_playing_format.file", guild_id, title=display_title
        )
    )
    now_playing_value = get_messages(
        ...
        artist=display_artist,  # Gunakan artist yang sudah dibersihkan
...
async def update_controller(
    ...
    Includes validation to handle deleted/inaccessible channels.
    """
    current_time = time.time()
    if guild_id in controller_update_cooldown:
        if current_time - controller_update_cooldown[guild_id] < 2.0:
            return

    controller_update_cooldown[guild_id] = current_time

    controller_channel_id = get_guild_state(guild_id).controller_channel_id

    # Validate controller channel exists and is accessible
    if controller_channel_id:
        channel = bot.get_channel(controller_channel_id)
        if not channel:
            logger.warning(
                f"Controller channel {controller_channel_id} not found for guild {guild_id}. Resetting controller settings."
            )
            # Reset controller settings when channel is not found
            get_guild_state(guild_id).controller_channel_id = None
            get_guild_state(guild_id).controller_message_id = None

            # If we're responding to an interaction, inform the user
    ...
    # If no controller is set up and no interaction, skip
    ...
    try:
        ...
        if interaction:
            # Scenario 1: Responding directly to a command
            await interaction.edit_original_response(
                content=None, embed=embed, view=view
            )
            message = await interaction.original_response()

            # Delete old controller message to avoid duplicates
            ...
            if message_id:
                ...
                # No message ID stored, create a new one
                new_message = await channel.send(embed=embed, view=view, silent=True)
                get_guild_state(guild_id).controller_message_id = new_message.id

    except discord.Forbidden:
        logger.error(
            f"Missing permissions to access controller channel {controller_channel_id} in guild {guild_id}. Resetting controller."
        )
        get_guild_state(guild_id).controller_channel_id = None
        get_guild_state(guild_id).controller_message_id = None
    except Exception as e:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not song:
                await interaction.followup.send(
                    get_messages(
                        "lyrics.error.refine_failed", self.guild_id, query=new_query
                    ),
                    silent=SILENT_MESSAGES,
                    ephemeral=True,
                )
            ...
            # Ambil message dari original interaction
            message = await self.original_interaction.original_response()
            await message.edit(embed=final_embed, view=new_view)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not lyrics_lines:
            fail_message = get_messages(
                "karaoke.not_found_description", self.guild_id
...
async def update_voice_channel_status(guild_id: int, status_text: str = None):
    """
    Mengupdate status voice channel dengan judul lagu yang sedang diputar
    """
    try:
        state = get_guild_state(guild_id)
        music_player = state.music_player
        ...
        if not getattr(state, "auto_channel_status", True):
            return

        if not music_player.voice_client or not music_player.voice_client.channel:
            return

        voice_channel = music_player.voice_client.channel

        # Jika tidak ada status yang diberikan, hapus status
        if not status_text:
            await voice_channel.edit(status=None)
            logger.info(f"[{guild_id}] Voice channel status cleared")
            return

        # Batasi panjang status (Discord limit: 500 karakter)
        if len(status_text) > 500:
            status_text = status_text[:497] + "..."

        await voice_channel.edit(status=status_text)
        logger.info(f"[{guild_id}] Voice channel status updated: {status_text}")

    except discord.Forbidden:
        logger.warning(f"[{guild_id}] No permission to edit voice channel status")
    except Exception as e:
        logger.error(f"[{guild_id}] Error updating voice channel status: {e}")
...
async def hydrate_track_info(music_player, track_info: dict, guild_id: int) -> dict:
    """
    ...
    if isinstance(track_info, dict):
        ...
        if track_info.get("title") and track_info.get("title") != get_messages(
            "player.loading_placeholder", guild_id
        ):
            return track_info
...
def get_track_display_info(track, guild_id: int = None) -> dict:
    """Normalizes track information access."""
    if guild_id is None:
        guild_id = 0  # Fallback for missing guild_id
    ...
    if isinstance(track, LazySearchItem):
        if track.resolved_info and not track.resolved_info.get("error"):
            ...
            title_to_display = track.title or get_messages(
                "player.loading_placeholder", guild_id
            )
            return {
...
def create_loading_bar(progress, width=12):
    """Creates a modern animated progress bar."""
    try:
    ...
    # Animated blocks
    blocks = ["▱", "▰"]
    bar = "".join([blocks[1] if i < filled else blocks[0] for i in range(width)])
    ...
    return f"{indicator} `{bar}` **{percentage}%**"
    ...
    if sp:
        try:
            ...
            if "playlist" in clean_url:
                ...
                results = await loop.run_in_executor(
                    None, lambda: sp.album_tracks(clean_url, limit=7)
                )
                while results:
    ...
    try:
        if not (is_a_loop or seek_time > 0):
            ...
            if music_player.queue.empty():
                ...
                # Di dalam fungsi play_audio, bagian autoplay (sekitar line 3726)
                elif (
                    get_guild_state(guild_id)._24_7_mode
                        ...
                        try:
                            if not progress_message and music_player.text_channel:
                                initial_embed = Embed(
                                    ...
                                    color=(
                                        0xC7CEEA if is_kawaii else discord.Color.blue()
                                    ),
                                )
                                progress_message = await music_player.text_channel.send(
                            ...
                            if "youtube.com" in seed_url or "youtu.be" in seed_url:
                                ...
                                if mix_playlist_url:
                                    ...
                                    if info.get("entries"):
                                        ...
                                        # UBAH: Ambil 50 lagu tapi hanya tambahkan 10
                                        all_recommendations = [
                                            entry
                                ...
                                if station_url:
                                    ...
                                    if (
                                        ...
                                        # UBAH: Ambil semua tapi hanya tambahkan 10
                                        recommendations = info["entries"][
                                            1:11
                                        ]  # BATASI HANYA 10 LAGU (skip index 0, ambil 1-10)

                            if recommendations and progress_message:
                                total_to_add = len(
                                    recommendations
                                )  # Ini akan maksimal 10
                                original_requester = (
                                ...
                                for i, entry in enumerate(recommendations):
                                    ...
                                    # Update progress setiap 5 lagu atau di akhir
                                    if (i + 1) % 5 == 0 or (i + 1) == total_to_add:
                                        progress = (i + 1) / total_to_add
                                        updated_embed = progress_message.embeds[0]
                                        updated_embed.description = get_messages(
                                            "autoplay_loading_description", guild_id
                                        ).format(
                                            progress_bar=create_loading_bar(progress),
        ...
        if music_player.current_info.get("source_type") != "file":
            ...
            try:
                # RETRY LOGIC untuk refresh URL
                for attempt in range(3):
                    try:
                        refreshed_info = await asyncio.wait_for(
                            fetch_video_info_with_retry(url_for_fetching), timeout=15.0
                        )
                        ...
                        if attempt == 2:
                            raise
                        logger.warning(
                            f"[{guild_id}] Refresh attempt {attempt+1} timed out, retrying..."
                        )
        ...
        if music_player.current_info and not is_a_loop and seek_time == 0:
            title = music_player.current_info.get("title", "Unknown Title")
            artist = music_player.current_info.get("uploader", "Unknown Artist")
            ...
            for remove_text in [
                ...
                clean_title = clean_title.replace(remove_text, "").strip()

            display_title = clean_display_title(clean_title)
            display_artist = clean_display_title(artist)

            # **PERUBAHAN UTAMA: Cek apakah nama artis sudah ada di judul**
            # Gunakan pengecekan case-insensitive untuk kecocokan yang lebih baik
            artist_already_in_title = display_artist.lower() in display_title.lower()

            # Jika artis sudah ada di judul, tampilkan judul saja
            if artist_already_in_title or display_artist == "Unknown Artist":
                status_text = f"🎶 **{display_title}**"
            # Jika artis belum ada di judul, tampilkan keduanya
            elif len(f"**{display_title}** - {display_artist}") <= 476:
                status_text = f"🎶 **{display_title}** - {display_artist}"
            else:
                # Jika terlalu panjang dengan artis, tampilkan judul saja
                status_text = f"🎶 **{display_title}**"

            bot.loop.create_task(update_voice_channel_status(guild_id, status_text))
...
async def play(interaction: discord.Interaction, query: str):
    ...
    try:
        ...
        # Case 1: Platforms requiring conversion (Spotify, etc.)
        ...
        # Case 2: Direct platforms (SoundCloud, YouTube, Bandcamp, .mp3 links)
        if (
            DIRECT_LINK_REGEX.match(query)
            or YOUTUBE_REGEX.match(query)
            ...
            if "entries" in info and len(info["entries"]) > 1:
                ...
                # HYDRATION LOGIC DARI VERSI LAMA
                tracks_to_hydrate = [
                    t
                    for t in tracks_to_add
                    if isinstance(t, dict)
                    and (
                        not t.get("duration", 0) > 0 or "video #" in t.get("title", "")
                    )
                ]

                if tracks_to_hydrate:
                    logger.info(
                        f"Hydrating {len(tracks_to_hydrate)} tracks for playlist"
                    )
                    tasks = [
        ...
        try:
            await interaction.edit_original_response(
                content=get_messages("error.command.fallback", guild_id, error=str(e)),
                embed=None,
                view=None,
            )
...
async def now_playing(interaction: discord.Interaction):
    ...
    if music_player.current_info:
        ...
        if music_player.current_info.get("source_type") == "file":
            ...
            description_text = get_messages("now_playing_description", guild_id).format(
                title=title, url=url
            )
...
async def skip(
    ...
    if next_song_info:
        ...
        if isinstance(next_song_info, LazySearchItem):
            if not next_song_info.resolved_info:
                await next_song_info.resolve()
            hydrated_next_info = next_song_info.resolved_info or {
                "title": "Unknown Title",
                "url": "#",
            }
        elif isinstance(next_song_info, dict):
            hydrated_next_info = next_song_info
            # If title is missing or placeholder, try to fetch it
            if (
                not hydrated_next_info.get("title")
                or hydrated_next_info.get("title") == "Unknown Title"
            ):
                try:
                    url_to_fetch = hydrated_next_info.get("url")
                    if url_to_fetch:
                        full_info = await fetch_video_info_with_retry(url_to_fetch)
                        hydrated_next_info.update(full_info)
                except Exception as e:
                    logger.error(f"Failed to hydrate next song info: {e}")
        else:
            hydrated_next_info = {"title": "Unknown Title", "url": "#"}
...
async def radio_24_7(interaction: discord.Interaction, mode: str):
    ...
    if not voice_client:
        if music_player.text_channel:
            await music_player.text_channel.send(
                get_messages("error.connection.title", guild_id), silent=SILENT_MESSAGES
            )
        return
...
async def on_voice_state_update(member, before, after):
    ...
    if member.id == bot.user.id and after.channel is None:
        ...
        if get_guild_state(guild_id)._24_7_mode:
            logger.warning(
                f"Bot was disconnected from guild {guild_id}, but 24/7 mode is active. Attempting auto-reconnect."
            )
            ...
            if not last_channel:
                logger.error(
                    f"Cannot auto-reconnect for guild {guild_id}: no previous channel info."
                )
                music_player.voice_client = None
                return

            # Simpan timestamp playback
            current_timestamp = music_player.start_time
            if music_player.playback_started_at:
                elapsed = time.time() - music_player.playback_started_at
                current_timestamp += elapsed * music_player.playback_speed

            # Cancel task lama
            if music_player.current_task and not music_player.current_task.done():
                music_player.current_task.cancel()

            # Reconnect dan resume
            async def reconnect_24_7():
                try:
                    await asyncio.sleep(2)  # Beri waktu Discord untuk cleanup
                    new_vc = await last_channel.connect()
                    music_player.voice_client = new_vc
                    logger.info(
                        f"Successfully reconnected to guild {guild_id} in 24/7 mode."
                    )
                    ...
                    logger.error(f"Failed to auto-reconnect for guild {guild_id}: {e}")
                    music_player.voice_client = None

            bot.loop.create_task(reconnect_24_7())
            return

...
class LazySearchItem:
    ...
    async def resolve(self):
        async with self.search_lock:
            ...
            logger.error(f"[LazyResolve] Failed '{self.title} {self.artist}': {last_exc}")
...
async def process_tidal_url(url, interaction):
    # (Unchanged from original — Playwright-based; keeping the same logic)
    guild_id = interaction.guild.id
    browser = None
    try:
        clean = url.split("?")[0]
